...
export function Workspace(_props?: WorkspaceProps) {
  ...
  // Фильтр по диапазону дат публикации (taken_at). Храним как 'YYYY-MM-DD' для date-инпутов.
  const [dateFrom, setDateFrom] = useState<string>(() => {
    try { return localStorage.getItem('workspace_dateFrom') || ''; } catch { return ''; }
  });
  const [dateTo, setDateTo] = useState<string>(() => {
    try { return localStorage.getItem('workspace_dateTo') || ''; } catch { return ''; }
  });
  const [showDateRange, setShowDateRange] = useState(false);
  // Конвертация в unix-секунды для запроса (от — начало дня, до — конец дня включительно)
  const dateFromTs = dateFrom ? Math.floor(new Date(`${dateFrom}T00:00:00`).getTime() / 1000) : null;
  const dateToTs = dateTo ? Math.floor(new Date(`${dateTo}T23:59:59`).getTime() / 1000) : null;
  ...
  useEffect(() => {
    try { localStorage.setItem('workspace_dateFrom', dateFrom); } catch { /* ignore */ }
  }, [dateFrom]);
  useEffect(() => {
    try { localStorage.setItem('workspace_dateTo', dateTo); } catch { /* ignore */ }
  }, [dateTo]);
  ...
  const getSortedVideos = (videos: ZoneVideo[]): ZoneVideo[] => {
    ...
    return [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'date': {
          // Дата публикации: taken_at — unix-секунды (число); если нет — фолбэк на created_at (ISO)
          const ts = (v: ZoneVideo): number => {
            if (v.taken_at != null) return Number(v.taken_at) || 0;
            const t = v.created_at ? new Date(v.created_at).getTime() : 0;
            return isNaN(t) ? 0 : Math.floor(t / 1000);
          };
          return tieBreak(a, b, ts(b) - ts(a));
        }
        case 'recent':
          return tieBreak(
            a,
          ...
          <div className="mb-6 md:mb-8 rounded-2xl md:rounded-card-xl bg-white/72 backdrop-blur-glass-xl shadow-glass border border-white/55 px-4 py-4 md:px-6 md:py-5 relative z-20">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-5">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {/* Фильтр по диапазону дат публикации — кнопка-пилюля + поповер */}
                <div className="relative">
                  <button
                    onClick={() => setShowDateRange(v => !v)}
                    className={cn(
                      "flex items-center gap-1.5 text-xs rounded-pill px-3 py-1.5 backdrop-blur-glass shadow-glass-sm border transition-all",
                      hasDateRange
                        ? "bg-slate-700 text-white border-slate-700"
                        : "bg-white/62 text-slate-600 border-white/55 hover:bg-white/80"
                    )}
                  >
                    <Calendar className="w-3.5 h-3.5" strokeWidth={2.5} />
                    <span>
                      {(() => {
                        const fmt = (s: string) => new Date(`${s}T00:00:00`).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' });
                        if (dateFrom && dateTo) return `${fmt(dateFrom)} – ${fmt(dateTo)}`;
                        if (dateFrom) return `с ${fmt(dateFrom)}`;
                        if (dateTo) return `до ${fmt(dateTo)}`;
                        return 'Период';
                      })()}
                    </span>
                    {hasDateRange && (
                  ...
                  {showDateRange && (
                    <>
                      <div className="fixed inset-0 z-[500]" onClick={() => setShowDateRange(false)} />
                      <div className="absolute left-0 top-full mt-1.5 z-[501] bg-white border border-slate-200/90 rounded-card-xl shadow-xl p-3 w-[260px] animate-in fade-in slide-in-from-top-1 duration-150">
                        <div className="flex flex-col gap-2.5">
                          <label className="flex items-center justify-between gap-2 text-xs text-slate-600">
                            ...
                            <input
                              ...
                              className="px-2 py-1.5 rounded-lg border border-slate-200 text-slate-800 text-sm bg-white outline-none focus:ring-2 focus:ring-slate-200/70"
                            />
                          </label>
                          <label className="flex items-center justify-between gap-2 text-xs text-slate-600">
                          ...
                          <div className="flex flex-wrap gap-1.5 pt-1 border-t border-slate-100">
                            {[
                              ...
                              <button
                                ...
                                className="px-2.5 py-1 rounded-lg text-xs text-slate-600 bg-slate-50 hover:bg-slate-100 transition-colors"
                              >
                                {label}
                              </button>
                            ))}
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg text-xs text-slate-500 hover:bg-slate-100 transition-colors ml-auto"
                            >
                              Сбросить
                            </button>
                          </div>
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {sortBy === 'date' && <><Calendar className="w-3.5 h-3.5 text-slate-500" strokeWidth={2.5} /><span>По дате</span></>}
